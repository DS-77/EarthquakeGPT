...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(training_data)):
        ...
        if fold == 4:
            ...
            mae = mean_absolute_error(val_df['y'].values, pred['LSTM'][:2884].values)
            mse = mean_squared_error(val_df['y'].values, pred['LSTM'][:2884].values)
            rmse = root_mean_squared_error(val_df['y'].values, pred['LSTM'][:2884].values)
            mape = mean_absolute_percentage_error(val_df['y'].values, pred['LSTM'][:2884].values)
            r2 = r2_score(val_df['y'].values, pred['LSTM'][:2884].values)
        else:
            mae = mean_absolute_error(val_df['y'].values, pred['LSTM'].values)
            mse = mean_squared_error(val_df['y'].values, pred['LSTM'].values)
            rmse = root_mean_squared_error(val_df['y'].values, pred['LSTM'].values)
            mape = mean_absolute_percentage_error(val_df['y'].values, pred['LSTM'].values)
            r2 = r2_score(val_df['y'].values, pred['LSTM'].values)
    ...
    lstm_model.save(path=weights_path, model_index=None, overwrite=True, save_dataset=True)
    ...
    # Print the results
    print()
    print("Training Five Fold Results: LSTM")
    print("=" * 80)
    ...
    with open(final_filename, 'w') as f_file:
        f_file.write("Testing Results: LSTM\n")
        f_file.write("=" * 80)

...
def train_kan_model(train_df, val_df):
    """
    ...
    # Train model
    kan_model.fit(train_df, val_size=577)
    pred = pd.DataFrame()
...
def five_fold_cross_validation(training_data, weights_path, plot_path, results_path):
    ...
    for fold, (train_index, val_index) in enumerate(tscv.split(training_data)):
        ...
        pred, kan_model = train_kan_model(train_df, val_df)
        last_trained_model = kan_model

        # Calculate performance metrics
        # Handle potential length discrepancies with val_df and pred
        y_true = val_df['y'].values
        y_pred = pred['KAN'][:len(y_true)]
...
def main():
    ...
    ap.add_argument("-w", "--weights", type=str, required=False, default="runs/kan/weights",
    ...
    weights_path = "runs/kan/weights/"
    results_path = "runs/kan/results"
    plot_path = "runs/kan/five_fold"
    ...
    if mode == "train":
        ...
        kan_model = NeuralForecast.load(pretrain_weights)
        testing_results = evaluate_on_test_data(kan_model, testing_data, results_path)
        print("Done.")
    else:

"""
This module implements the PatchTST model for forecasting earthquake magnitudes.
...
def train_patchtst_model(train_df, val_df):
    """
    ...
    """
    h = 577
    patch_model = NeuralForecast(models=[PatchTST(h=h,
                                                  input_size=6,
                                                  patch_len=24,
                                                  stride=24,
                                                  revin=False,
    ...
    # Train model
    patch_model.fit(train_df, val_size=577)
    pred = pd.DataFrame()

    # Predict values
    for i in range(0, len(val_df), 577):
        temp_val_chunk = val_df.iloc[i:i + 577]
        temp_pred = patch_model.predict(temp_val_chunk).reset_index()
        pred = pd.concat([pred, temp_pred], ignore_index=True)

    return pred, patch_model
...
def five_fold_cross_validation(training_data, weights_path, plot_path, results_path):
    ...
    for fold, (train_index, val_index) in enumerate(tscv.split(training_data)):
        ...
        pred, patch_model = train_patchtst_model(train_df, val_df)
        last_trained_model = patch_model

        # Calculate performance metrics
        # Handle potential length discrepancies with val_df and pred
        y_true = val_df['y'].values
        y_pred = pred['PatchTST'][:len(y_true)]
...
def plot_five_fold_performance(avg_MAE, avg_MSE, avg_RMSE, avg_MAPE, avg_r2, plot_path):
    ...
    plt.tight_layout()
    plt.savefig(f"{plot_path}/patchtst_five_fold_plot.png")
...
def save_five_fold_results(avg_MAE, avg_MSE, avg_RMSE, avg_MAPE, avg_r2, results_path):
    ...
    """
    ff_filename = f"{results_path}/patchtst_ff_results.txt"
    with open(ff_filename, 'w') as ff_file:
        ...
        ff_file.write("\n")
        ff_file.write("Training Five Fold Results: PatchTST\n")
        ff_file.write("=" * 80)
...
def evaluate_on_test_data(patch_model, testing_data, results_path):
    """
    Evaluate the trained model on testing data.
    :param patch_model: (NeuralForecast) Trained model
    ...
    for b in range(b_num - 1):
        ...
        temp_pred = patch_model.predict(temp_chunk).reset_index()
    ...
    last_pred = patch_model.predict(last_chunk)
    last_pred = last_pred.iloc[:144]
    test_pred = pd.concat([test_pred, last_pred], ignore_index=True)

    # Calculate the metrics
    test_mae = mean_absolute_error(testing_data['y'].values, test_pred['PatchTST'].values)
    test_mse = mean_squared_error(testing_data['y'].values, test_pred['PatchTST'].values)
    test_rmse = root_mean_squared_error(testing_data['y'].values, test_pred['PatchTST'].values)
    test_mape = mean_absolute_percentage_error(testing_data['y'].values, test_pred['PatchTST'].values)
    test_r2 = r2_score(testing_data['y'].values, test_pred['PatchTST'].values)
    ...
    final_filename = f"{results_path}/patchtst_final_result.txt"
...
def main():
    ...
    ap.add_argument("-w", "--weights", type=str, required=False, default="runs/patch_tst/weights",
    ...
    weights_path = "runs/patch_tst/weights/"
    results_path = "runs/patch_tst/results"
    plot_path = "runs/patch_tst/five_fold"
    ...
    if mode == "train":
        ...
        patch_model = NeuralForecast.load(pretrain_weights)
        testing_results = evaluate_on_test_data(patch_model, testing_data, results_path)
        print("Done.")
    else:

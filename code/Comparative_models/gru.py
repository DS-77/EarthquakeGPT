...
def train_gru_model(train_df, val_df):
    """
    ...
    # Train model
    gru_model.fit(train_df, val_size=577)
    pred = pd.DataFrame()
...
def five_fold_cross_validation(training_data, weights_path, plot_path, results_path):
    ...
    for fold, (train_index, val_index) in enumerate(tscv.split(training_data)):
        ...
        pred, gru_model = train_gru_model(train_df, val_df)
        last_trained_model = gru_model

        # Calculate performance metrics
        # Handle potential length discrepancies with val_df and pred
        y_true = val_df['y'].values
        y_pred = pred['GRU'][:len(y_true)]
...
def evaluate_on_test_data(gru_model, testing_data, results_path):
    """
    Evaluate the trained model on testing data.
    :param gru_model: (NeuralForecast) Trained model
...
def main():
    ...
    ap.add_argument("-w", "--weights", type=str, required=False, default="runs/GRU/weights",
    ...
    if mode == "train":
        ...
        gru_model = NeuralForecast.load(pretrain_weights)
        testing_results = evaluate_on_test_data(gru_model, testing_data, results_path)
        print("Done.")
    else:

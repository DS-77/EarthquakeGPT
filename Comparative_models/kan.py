"""
This module implements the Kolmogorov-Arnold Network (KAN) model for forecasting earthquake magnitudes.
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(training_data)):
        ...
        kan_model = NeuralForecast(
            models=[KAN(h=577, input_size=6,
        ...
        # Train model
        kan_model.fit(train_df)
        pred = pd.DataFrame()

        # Predict values
        for i in range(0, len(val_df), 577):
            temp_val_chunk = val_df.iloc[i:i+577]
            temp_pred = kan_model.predict(temp_val_chunk).reset_index()
            pred = pd.concat([pred, temp_pred], ignore_index=True)

        # Calculate metrics
        if fold == 4:
            # Case for the last k-fold split: 4x Groups of 2885 and 1x Group 2884
            mae = mean_absolute_error(val_df['y'].values, pred['KAN'][:2884].values)
            mse = mean_squared_error(val_df['y'].values, pred['KAN'][:2884].values)
            rmse = root_mean_squared_error(val_df['y'].values, pred['KAN'][:2884].values)
            mape = mean_absolute_percentage_error(val_df['y'].values, pred['KAN'][:2884].values)
            r2 = r2_score(val_df['y'].values, pred['KAN'][:2884].values)
        else:
            mae = mean_absolute_error(val_df['y'].values, pred['KAN'].values)
            mse = mean_squared_error(val_df['y'].values, pred['KAN'].values)
            rmse = root_mean_squared_error(val_df['y'].values, pred['KAN'].values)
            mape = mean_absolute_percentage_error(val_df['y'].values, pred['KAN'].values)
            r2 = r2_score(val_df['y'].values, pred['KAN'].values)
    ...
    kan_model.save(path=weights_path, model_index=None, overwrite=True, save_dataset=True)
    ...
    plt.tight_layout()
    plt.savefig(f"{plot_path}/kan_five_fold_plot.png")
    # plt.show()

    # Print the results
    print()
    print("Training Five Fold Results: KAN")
    print("=" * 80)
    ...
    ff_filename = f"{results_path}/kan_ff_results.txt"
    with open(ff_filename, 'w') as ff_file:
        ...
        ff_file.write("\n")
        ff_file.write("Training Five Fold Results: KAN\n")
        ff_file.write("=" * 80)
    ...
    for b in range(b_num - 1):
        ...
        temp_pred = kan_model.predict(temp_chunk).reset_index()
    ...
    last_pred = kan_model.predict(last_chunk)
    last_pred = last_pred.iloc[:144]
    test_pred = pd.concat([test_pred, last_pred], ignore_index=True)

    # Calculate the metrics
    test_mae = mean_absolute_error(testing_data['y'].values, test_pred['KAN'].values)
    test_mse = mean_squared_error(testing_data['y'].values, test_pred['KAN'].values)
    test_rmse = root_mean_squared_error(testing_data['y'].values, test_pred['KAN'].values)
    test_mape = mean_absolute_percentage_error(testing_data['y'].values, test_pred['KAN'].values)
    test_r2 = r2_score(testing_data['y'].values, test_pred['KAN'].values)

    # Save Final result to a file
    final_filename = f"{results_path}/kan_final_result.txt"
